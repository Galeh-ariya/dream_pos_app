...
class FormPositionScreen extends StatefulWidget {
  const FormPositionScreen({
    super.key,
  ...
  @override
  State<FormPositionScreen> createState() => _FormPositionScreenState();
}

class _FormPositionScreenState extends State<FormPositionScreen> {
  final GlobalKey<FormState> _formKey = GlobalKey<FormState>();
  final TextEditingController _positionNameController = TextEditingController();
  String? _selectedOutletId;

  bool get _isEditMode =>
      widget.initialPositionName != null &&
      widget.initialPositionName!.trim().isNotEmpty;

  @override
  void initState() {
    super.initState();
    _positionNameController.text = widget.initialPositionName ?? '';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        titleSpacing: 0,
        title: Text(
          _isEditMode ? 'Form Edit Jabatan' : 'Form Jabatan',
      ...
      body: BlocConsumer<StorePositionBloc, StorePositionState>(
        listener: (context, state) {
          state.whenOrNull(
            success: (message) {
              ...
              Navigator.of(
                context,
              ).pop({'name': _positionNameController.text.trim()});
            },
            error: (message) {
              if (!mounted) return;
              ScaffoldMessenger.of(
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    _isEditMode ? 'Edit Jabatan' : 'Tambah Jabatan',
                    style: GoogleFonts.inter(
                      fontSize: 26,
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(
                          width: double.infinity,
                          child: FilledButton(
                            onPressed: isSaving ? null : _savePosition,
  ...
  Future<void> _savePosition() async {
    ...
    if (outletId == null || outletId.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Outlet tidak valid. Gagal menyimpan jabatan.'),
        ),
      );
      return;
    }
    ...
    context.read<StorePositionBloc>().add(
      StorePositionEvent.storePosition(outletId, positionName),
    );
  }
